...
const API_BASE_URL = 'http://192.168.0.209:5000/api';
...
const EngageActivityManagement = () => {
  ...
  const activityCategories = ['wellness', 'fitness', 'event'];
  const activityTypes = {
  ...
  const loadActivities = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/admin/engage/activities?category=${category}&page=1&limit=100`,
  ...
  const handleSaveActivity = async () => {
    try {
      ...
      if (editingItem) {
        await axios.put(
          `${API_BASE_URL}/admin/engage/activities/${editingItem.id}`,
          formDataToSend,
          {
            headers: {
        ...
        await axios.post(
          `${API_BASE_URL}/admin/engage/activities`,
          formDataToSend,
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleDeleteActivity = async (id) => {
    ...
    try {
      ...
      await axios.delete(
        `${API_BASE_URL}/admin/engage/activities/${id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
